<template>
  <v-row>
    <v-col lg="2" md="3" sm="4" xs="6" v-for="(product, index) in data_product">
      <v-tooltip top>
        <template v-slot:activator="{ on, attrs }">
          <v-card ripple="true" class="produk_card" v-bind="attrs" v-on="on">
            <v-card-actions>
              <v-img :src="require(`@/assets/images/produk/convert/${product.thumbnail}`)"></v-img>
            </v-card-actions>
            <v-card-text align="center" class="produk_title">
              {{ product.title }}
            </v-card-text>
          </v-card>
        </template>
  ...
  data() {
    return {
      data_product: [
        ...
        { id: 7, title: "Nasi Goreng Spesial", thumbnail: "bakso.jpg", price: 18000, categoryId: 1 },
